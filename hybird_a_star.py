...
from collision_check import two_circle_check, distance_check
...
class hybrid_a_star:
    ...
    def __init__(self,
        ...
        _ , self.h_value_list = self.heuristic.compute_path(node_x=park_map.case.x0, node_y=park_map.case.y0)
        ...
        for i in range(next_index):
            ...
            if find_opennode == False:
                ...
                for i in range(math.ceil(self.dt / self.ddt)):
                    ...
                    theta_i = current_node.theta + (self.vehicle.max_v * np.tan(steering_angle)) / self.vehicle.lw * self.ddt * (i+1)
                    ...
                    if self.config['collision_check'] == 'circle':
                        collision = two_circle_check(node_x=x_i,node_y=y_i,theta= theta_i, map=self.park_map)
                    else:
                        collision = distance_check(node_x=x_i,node_y=y_i,theta= theta_i,
                                                   map=self.park_map, config=self.config)
                    if collision:
                ...
                if not collision:
                ## caculate cost
                    child_node.g = self.calc_node_cost(child_node, father_theta=current_node.theta, father_gear=current_node.forward)
                ## caculate heuristic
            ...
            # if this node has been explored 
                ...
                new_g = self.calc_node_cost(child_node, father_theta=current_node.theta, father_gear=current_node.forward)
                new_f = new_h + new_g
    ...
    def calc_node_heuristic(self, current_node:Node)-> np.float64:
        ...
        for i in range(len(self.h_value_list)):
            ...
            if find_x and find_y:
                ...
                break
        if find_grid == False:
            h_value_1, self.h_value_list = self.heuristic.compute_path(node_x=current_node.x, node_y=current_node.y)
    ...
    def try_reach_goal(self, current_node:Node) -> bool:
        ...
        in_radius = False
        collision_p = None
        distance = np.sqrt((current_node.x - self.goal_node.x)**2+(current_node.y-self.goal_node.y)**2)
    ...
    def try_rs_curve(self, current_node:Node):
        ...
        for i in range(len(rs_path.x)):
            ...
            if self.config['collision_check'] == 'circle':
                collision = two_circle_check(node_x=path_x,
                                       node_y=path_y,
                                       theta=path_theta,
                                       map=self.park_map)
            else:
                collision = distance_check(node_x=path_x,
                                           node_y=path_y,
                                           theta=path_theta,
                                           map=self.park_map,
                                           config=self.config)
    ...
    def finish_path(self, current_node:Node):
        ...
        for i in range(len(all_path_node)):
            ...
            for j in range(math.ceil(self.dt/self.ddt)):
                ...
                theta_j = theta_0 + (self.vehicle.max_v * np.tan(steering_angle)) / self.vehicle.lw * self.ddt * (j+1)
