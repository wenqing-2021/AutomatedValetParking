...
if __name__ == '__main__':
    ...
    ## path planning
    original_path, ego_vehicle, path_info, split_path = path_planning(config, park_map)

    ## velocity planning
    velocity_planner = velocity_plan.velocity_planner(vehicle=ego_vehicle)

    ## path optimization (optimize the splited path)
    
    plot_opt_path =[] # store the optimization path
    
    plot_insert_path = [] # store the interpolation path

    optimizer = path_optimazition.path_opti(park_map, ego_vehicle, config)
    insert_point = cubic_interpolation.interpolation(config=config, map=park_map, vehicle= ego_vehicle)
    for path_i in split_path:
        opti_path = optimizer.get_result(path_i)
        velocity_func, acc_func = velocity_planner.solve_nlp(path=opti_path)
        insert_path = insert_point.get_cubic_interpolation(opti_path, velocity_func, acc_func)
