...
def get_near_obstacles(node_x,node_y,theta, map:_map, config:dict=None):
    '''
...
def two_circle_check(node_x, node_y, theta, map:_map) -> bool:
    ...
    # determine x
    near_x_position = obstacle_position_x[np.where((obstacle_position_x > left) & (obstacle_position_x < right))]
    near_y_position = obstacle_position_y[np.where((obstacle_position_x > left) & (obstacle_position_x < right))]
    # determine y
    near_obstacle_x = near_x_position[np.where((near_y_position > down) & (near_y_position < upper))]
    near_obstacle_y = near_y_position[np.where((near_y_position > down) & (near_y_position < upper))]
    # check these points
    collision = False
